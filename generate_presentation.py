...
class SubsidyPlatformPresentation:
    ...
    def add_case_studies_slide(self):
        ...
        insights = [
            ("启示一（建行案例）", "\"政府搭台、银行助力\"模式可快速起量，我行手机应用可作为核心载体"),
    ...
    def add_overall_solution_slide(self):
        ...
        components = [
            ("客户端用户入口", "\"补贴申请\"小程序（内嵌于招行手机应用或微信生态）", "便捷的用户申请体验"),
            ("管理端后台", "补贴申请审核平台（供银行和商务局人员使用）", "高效的审核管理工具"),
    ...
    def add_user_flow_slide(self):
        ...
        steps = [
            ("步骤1：入口/授权", "从招行手机应用/小程序进入，一键授权登录"),
            ("步骤2：车辆信息录入", "光学字符识别技术识别行驶证/购车发票，用户确认"),
            ("步骤3：证件验证", "光学字符识别技术识别人脸/身份证，与公安系统联网核验"),
            ("步骤4：绑定收款卡", 
             "• 智能引导：优先推荐绑定招行借记卡\n"
    ...
    def add_c_end_design_slide(self):
        ...
        features = [
            ...
            ("智能填报", "借助光学字符识别和大数据技术，自动识别和预填信息，减少用户操作，提升体验"),
    ...
    def add_b_end_design_slide(self):
        ...
        features = [
            ...
            ("一键审核", "审核员可一键\"通过\"或\"驳回\"\n驳回时需选择或填写原因，确保流程规范"),
            ("数据驾驶舱", "实时展示申请总量、审核进度、补贴发放总额、客群分析等核心关键绩效指标\n为项目决策提供数据支持，助力精准运营")
        ]
    ...
    def add_growth_strategy_slide(self):
        ...
        p = tf.add_paragraph()
        p.text = "文案：\"恭喜！您已获得若干万元招行备用金资格，购车消费更轻松！\""
        p.level = 2
        p.font.size = Pt(13)
        p.font.color.rgb = self.light_gray
        p.space_after = Pt(8)
        ...
        strategies = [
            ...
            ("手机应用引流", "整个流程设计优先在招行手机应用内完成，补贴发放后，通过手机应用消息推送引导用户参与其他活动（如本地生活优惠、理财产品推荐），提升手机应用月活"),
            ("业务闭环", "实现\"获客-活客-留客\"闭环：通过本次活动获取新客，通过手机应用互动激活客户，通过后续的综合金融服务留住客户")
        ]
    ...
    def add_value_summary_slide(self):
        ...
        bank_values = [
            ...
            ("商业价值", "批量获取了高质量车主客户，并成功带动了信用卡、贷款、手机应用活跃度等多项业务指标的增长")
        ]
